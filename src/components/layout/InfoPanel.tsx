    ...
    <aside className="w-[300px] flex-shrink-0 flex flex-col bg-bg-panel border-l border-border-subtle overflow-y-auto">
      ...
      <Section label="Identity binding">
        ...
        <p className="mt-1.5 text-[10px] text-text-tertiary leading-snug">
          The alias is a deterministic hash of the public identity key. A
          relay can't substitute a different bundle for this alias — the
          mismatch would fail verification on add.
        </p>
      </Section>
...
interface SecurityStatus {
  ...
  manifest_signed_url: string | null;
  tls_pin_hex: string | null;
  frame_counters: {
    frames_sent: number;
...
interface FrameAccountingPayload {
  verdict: string;
  severity: string;
  client_sent: number;
  client_received: number;
  relay_received_from_client: number;
  relay_sent_to_client: number;
}

function ConnectionIntegrity() {
  const [status, setStatus] = useState<SecurityStatus | null>(null);
  const [frameVerdict, setFrameVerdict] = useState<FrameAccountingPayload | null>(null);
  ...
  useEffect(() => {
    ...
    import("@tauri-apps/api/event").then(({ listen }) => {
      listen<FrameAccountingPayload>("security:frame_accounting", (e) =>
        setFrameVerdict(e.payload),
      ).then((u) => unlistens.push(u));
      listen<{ url: string }>("security:manifest_tampered", () =>
        setTampered(true),
      ).then((u) => unlistens.push(u));
      listen<{ url: string }>("security:tls_pin_mismatch", () =>
        setPinMismatch(true),
      ).then((u) => unlistens.push(u));
    });
    return () => {
  ...
  return (
    <div className="px-4 py-3 border-b border-border-subtle">
      ...
      />
      <Row
        label="Frame accounting (home)"
        value={
          frameVerdict == null ? (
            <Pill tone="off">Idle</Pill>
          ) : frameVerdict.severity === "ok" ? (
            <Pill tone="ok">Verified</Pill>
          ) : frameVerdict.severity === "critical" ? (
            <Pill tone="err">{frameVerdict.verdict}</Pill>
          ) : (
            <Pill tone="warn">{frameVerdict.verdict}</Pill>
          )
        }
      />
      <Row
        ...
        value={
          pinMismatch ? (
            <Pill tone="err">Mismatch</Pill>
          ) : status?.relay_url?.startsWith("ws://") ? (
            <Pill tone="off">Plaintext</Pill>
          ) : status?.tls_pin_hex ? (
            <span
              className="text-[10px] font-mono text-text-primary"
              title={status.tls_pin_hex}
            >
              {status.tls_pin_hex.slice(0, 8)}…{status.tls_pin_hex.slice(-4)}
            </span>
          ) : (
      ...
      />
      <p className="mt-1.5 text-[10px] text-text-tertiary leading-snug">
        If anyone redirects your connection or tampers with messages on the
        wire, you'll see it here. Red status means active interference.
      </p>
    </div>
  );
}
...
interface RelayUsage {
  ...
  retrieves_ok: number;
  retrieves_failed: number;
  last_status: string | null;
  last_at_ms: number | null;
}

function CrossRelayActivity() {
  const [rows, setRows] = useState<RelayUsage[]>([]);
  useEffect(() => {
    const tick = async () => {
      try {
        setRows(await invoke<RelayUsage[]>("cross_relay_stats"));
      } catch {
        /* ignore */
      }
    };
    ...
    return (
      <div className="px-4 py-3 border-b border-border-subtle">
        ...
        <p className="text-[10px] text-text-tertiary leading-snug">
          No cross-relay deposits yet. When you message a contact who's on a
          different relay, the activity shows up here.
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="px-4 py-3 border-b border-border-subtle">
        ...
        {rows.map((r) => (
          <div key={r.url} className="space-y-0.5">
            <div className="flex items-center justify-between">
              <span
                className="text-[11px] font-mono text-text-primary truncate max-w-[180px]"
                title={r.url}
              >
                {prettyRelayUrl(r.url)}
              </span>
              {r.last_status && <StatusPill status={r.last_status} />}
            </div>
            <div className="flex items-center gap-3 text-[10px] font-mono text-text-tertiary">
              <span>↑ {r.deposits_ok}{r.deposits_failed > 0 && <span className="text-status-warn"> ({r.deposits_failed} fail)</span>}</span>
              {r.retrieves_ok + r.retrieves_failed > 0 && (
                <span>↓ {r.retrieves_ok}{r.retrieves_failed > 0 && <span className="text-status-warn"> ({r.retrieves_failed} fail)</span>}</span>
              )}
            </div>
          </div>
        ))}
      ...
      <p className="mt-2 text-[10px] text-text-tertiary leading-snug">
        Each row is a relay other than your home that you've sent messages
        through. Failures here are the equivalent of a frame-accounting
        warning for one-shot deposits.
      </p>
    </div>
  );
}

function prettyRelayUrl(url: string): string {
  try {
    const u = new URL(url);
...
function StatusPill({ status }: { status: string }) {
  ...
  if (status === "tls_mismatch") return <Pill tone="err">Pin</Pill>;
  if (status === "timeout") return <Pill tone="warn">Timeout</Pill>;
  if (status === "rejected") return <Pill tone="err">Rejected</Pill>;
